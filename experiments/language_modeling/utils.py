...
def get_loaders(
    ...
    tokenizer = get_tokenizer(
        model_name=model_name,
        cache_dir=cache_dir,
        add_padding_token=True,     # needed for causal LM padding
    )
    ...
    # Same data again, but batch_size = 1 (for decoding single sentences)
    eval_train_loader = get_loader(
        model_name=model_name,
        tokenizer=tokenizer,
        batch_size=1,
        data_path=data_path,
    ...
    # Optionally narrow down to a small subset of indices
    if valid_indices is not None:
        subset_ds = eval_train_loader.dataset.select(valid_indices)
    ...
    test_loader = get_loader(
        ...
        data_path=data_path,
        data_name=data_name,
        split="validation",   # 'validation' obeys HF naming for Wikitext
        cache_dir=cache_dir,
    )
